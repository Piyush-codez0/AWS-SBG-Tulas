...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      const buttonEl = target.closest("button, .btn-primary, [role='button']");
      if (!buttonEl) return;

      const btnText = (buttonEl.textContent || "").toLowerCase();
      const isContinueOrSubmit =
        buttonEl.classList.contains("btn-primary") ||
        btnText.includes("continue") ||
        btnText.includes("deploy") ||
        btnText.includes("submit");

      if (!isContinueOrSubmit) return;
