...
function checkFullOpacity(el: HTMLElement | null): boolean {
...
function waitForFullOpacity(el: HTMLElement | null, callback: () => void): () => void {
...
function AnimateIcon({
  ...
  const { ref: inViewRef, isInView } = useIsInView(viewOuterRef, {
    inView: !!(animateOnView || animateOnHover),
  ...
  React.useEffect(() => {
    ...
    if (isInView) {
      ...
      if (checkFullOpacity(el)) {
        ...
        cancelWait = waitForFullOpacity(el, () => startAnimation(animateOnView));
      }
    } else {
      stopAnimation();
    }
  ...
  React.useEffect(() => {
    ...
    if (isMobile) {
      if (isInView) {
        ...
        if (checkFullOpacity(el)) {
          ...
          cancelWait = waitForFullOpacity(el, () => startAnimation(animateOnHover));
        }
      } else {
        stopAnimation();
      }
    ...
    if (el) {
      ...
      if (parentElement) {
        ...
        parentElement.addEventListener('mouseenter', handleParentEnter);
        parentElement.addEventListener('mouseleave', handleParentLeave);
      }
    }

    return () => {
      if (cancelWait) cancelWait();
      if (parentElement && handleParentEnter && handleParentLeave) {
        parentElement.removeEventListener('mouseenter', handleParentEnter);
        parentElement.removeEventListener('mouseleave', handleParentLeave);
      }
    };
  }, [animateOnHover, isInView, startAnimation, stopAnimation]);
